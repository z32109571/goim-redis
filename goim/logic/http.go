...
func Push(w http.ResponseWriter, r *http.Request) {
	...
	log.Info("subkeys is %v",subKeys)
	for serverId, keys = range subKeys {
		if err = mpushRedis(serverId, keys, bodyBytes); err != nil {
...
func PushRoom(w http.ResponseWriter, r *http.Request) {
	...
	if err = broadcastRoomRedis(int32(rid), bodyBytes, enable); err != nil {
	//if err = broadcastRoomKafka(int32(rid), bodyBytes, enable); err != nil {
		...
		log.Error("broadcastRoomRedis(\"%s\",\"%s\",\"%d\") error(%s)", rid, body, enable, err)
		res["ret"] = InternalErr
		return
	}
	res["ret"] = OK
...
func PushAll(w http.ResponseWriter, r *http.Request) {
	...
	body = string(bodyBytes)
	// push all
	if err := broadcastRedis(bodyBytes); err != nil {
	//if err := broadcastKafka(bodyBytes); err != nil {
		//log.Error("broadcastKafka(\"%s\") error(%s)", body, err)
		log.Error("broadcastRedis(\"%s\") error(%s)", body, err)
		res["ret"] = InternalErr
		return
	}
	res["ret"] = OK
